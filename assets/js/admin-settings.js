...
(function ($) {
    ...
    DomainManager.prototype.init = function () {
        ...
        this.$addBtn.on('click', function () {
            ...
            if (isSiteDomain(domain)) {
                alert('نمی‌توانید دامنه خود سایت را مسدود کنید.');
                return;
            }
...
function isSiteDomain(domain) {
    var siteHost = window.location.hostname.replace(/^www\./, '');
    var input = domain.replace(/^https?:\/\//, '').replace(/^www\./, '').split('/')[0].toLowerCase();
    return input === siteHost;
}
